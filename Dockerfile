...
# Install SSH Service RUN yum install -y openssh-server passwd
...
RUN yum -y install php56w php56w-fpm php56w-mbstring php56w-xml php56w-mysql php56w-pdo php56w-gd php56w-pecl-imagick 

php56w-opcache php56w-pecl-memcache php56w-pecl-xdebug


# Install php-mssql,mcrypt RUN rpm -Uvh https://dl.fedoraproject.org/pub/epel/epel-release-latest-6.noarch.rpm
...
# Install supervisor RUN yum -y install python-setuptools && \
...
RUN cp -p /usr/share/zoneinfo/Europe/Amsterdam /etc/localtime
